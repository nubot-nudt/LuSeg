...
parser.add_argument('--epoch_max', '-em', type=int, default=60)  # please stop training mannully
parser.add_argument('--epoch_from', '-ef', type=int, default=0)
parser.add_argument('--num_workers', '-j', type=int, default=8)
parser.add_argument('--n_class', '-nc', type=int, default=3)
parser.add_argument('--data_dir', '-dr', type=str, default='/home/shorwin/data/Lunar_vis')
parser.add_argument('--data_name', '-dn', type=str, default='LunarSeg')
parser.add_argument('--rgb_dir', '-rd', type=str, default='./weights_backup/LunarSeg/LunarSeg_RGB/196.pth')
...
def train(epo, model_t, model_s, contrastive_loss_fn, train_loader, optimizer,criterion):
    model_s.train()
    model_t.eval()
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        rgb_t, _,= model_t(images)
        rgb, depth, rgb_result = model_s(images)

        loss_constrast_depth = contrastive_loss_fn(depth, rgb_t)
        loss_r = criterion(rgb_result,labels)  # Note that the cross_entropy function has already include the softmax function



        loss = loss_r + loss_constrast_depth
        ...
        print(
            'Train: %s, epo %s/%s, iter %s/%s, lr %.8f, %.2f img/sec, loss %.4f, loss_r %.4f, loss_constrast_depth %.4f, time %s' \
            % (args.model_name, epo, args.epoch_max, it + 1, len(train_loader), lr_this_epo,
               len(names) / (time.time() - start_t), float(loss), float(loss_r),
               float(loss_constrast_depth),
...
def validation(epo, model, val_loader,criterion):
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(val_loader):
            ...
            _, _, rgb_result = model(images)  # depth image result, rgb image result, rgb image pre-segmentaion,  complement feature
            loss_r = criterion(rgb_result,labels)  # Note that the cross_entropy function has already include the softmax function
            loss = loss_r
...
if __name__ == '__main__':
    ...
    model_t = LuSeg_RGB(n_class=args.n_class)
    model_file = args.rgb_dir
    pretrained_weight = torch.load(model_file, map_location=lambda storage, loc: storage.cuda(args.gpu))
    own_state = model_t.state_dict()
    for name, param in pretrained_weight.items():
        own_state[name].copy_(param)
    print('done!')

    model_s = LuSeg_S(n_class=args.n_class)
    ...
    # preparing folders
    if os.path.exists("./runs_TS"):
        shutil.rmtree("./runs_TS")
    weight_dir = os.path.join("./runs_TS", args.model_name)
    os.makedirs(weight_dir)

    writer = SummaryWriter("./runs_TS/tensorboard_log")
    ...
    for epo in range(args.epoch_from, args.epoch_max):
        ...
        train(epo, model_t, model_s, contrastive_loss_fn, train_loader, optimizer,criterion)
